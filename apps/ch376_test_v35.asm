...
.read_disk_id_ok:
    ldd .msg_disk_wire[15:8]
    lde .msg_disk_wire[7:0]
    jsr ACIA_SEND_STRING
    lda CH376_SCRATCH
    jsr ch376_print_hex8
    jsr ch376_print_nl
...
.list_root_loop:
    ...
    beq .list_root_read
    cmp CH376_INT_SUCCESS
    beq .list_root_do_enum
    jmp .list_root_fail

.list_root_read:
    ; Tight path: RD under SEI inside proto — no console I/O before this.
    ; usb_timeout_off inside RD restores CH376_TMO; do not clobber A with .set_timeout.
    jsr .set_timeout_long
...
.list_root_rd_fail:
    ; Print diagnostics only after RD failed.
    ...
    jsr ch376_print_hex8
    jsr ch376_print_nl
    ldd .msg_wire_len[15:8]
    lde .msg_wire_len[7:0]
    jsr ACIA_SEND_STRING
    lda CH376_SCRATCH
    jsr ch376_print_hex8
    jsr ch376_print_nl
...
.list_root_finish:
    ...
    jsr ch376_print_hex8
    jsr ch376_print_nl
    ; If we got FAT bytes but printed nothing, dump name field as hex.
    lda CH376_FILE_COUNT
    bne .list_root_ok
    lda CH376_WIRE_LEN
    beq .list_root_ok
...
.msg_boot:
    #d 0x0A, 0x0D, "CH376 test v35", 0x0A, 0x0D, 0x00
.msg_title:
    #d "--- CH376S test v35 (ACIA2) ---", 0x0A, 0x0D, 0x00
.msg_ok:
...
CH376_LOADED_HI:
    #d 0x00
; Hot RX vars (BUF/CAP/…) at 0x8284 — see ch376_const.asm
