...
constexpr int RESOLUTION_DEFAULT           = 2;   // 800x600, 8 Colors (VGA8, ~100 cols with Auto font)
...
struct ConfDialogApp : public uiApp {
  ...
  void init() {
    ...
    // little help
    new uiStaticLabel(frame, "AdvancedTerminal for Project OTTO", Point(105, y), true, STYLE_LABELHELP2);
  ...
  static TermType getTermType() {
    return (TermType) preferences.getInt(PREF_TERMTYPE, (int) fabgl::ANSI_VT);  // default ANSI (native VT100/CSI) for Otto
  }


  static int getKbdLayoutIndex() {
    return preferences.getInt(PREF_KBDLAYOUT, 1);              // default 1 = Italian
  }
